...
namespace Bnan.Ui.Areas.CAS.Controllers
{
    ...
    public class CarStatisticsController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (Status_count < 2 && Branch_count < 2 && Brand_count < 2 && year_count < 2 && Category_count < 2 && Model_count < 2 && Status_count < 2)
            {
                return RedirectToAction("FailedMessageReport_NoData");
            }
            ...
            other.ArName = "أخرى  ";
            other.EnName = "  Others";
            other.ArName_for_table_Model = "أخرى  ";
            other.EnName_for_table_Model = "  Others";
            other.Code = "Aa";
        ...
        public async Task<IActionResult> GetAllByType(string Type, string listDrop, string singleNo)
        {
            ...
            if (Type == "Model")
            {
                var AllCars2_Model = _unitOfWork.CrCasCarInformation.FindAll(x => currentCar.CrMasUserInformationLessor == x.CrCasCarInformationLessor, new[] { "CrCasCarInformationModelNavigation", "CrCasCarInformationBrandNavigation" }).Where(x => x.CrCasCarInformationModelNavigation?.CrMasSupCarModelStatus != Status.Deleted).DistinctBy(x => x.CrCasCarInformationModel).ToList();
                ...
                foreach (var single in AllCars2_Model)
                {
                    ...
                    chartBranchDataVM.ArName_for_table_Model = single.CrCasCarInformationBrandNavigation?.CrMasSupCarBrandArName + " - " + single.CrCasCarInformationModelNavigation?.CrMasSupCarModelArName;
                    chartBranchDataVM.EnName_for_table_Model = single.CrCasCarInformationBrandNavigation?.CrMasSupCarBrandEnName + " - " + single.CrCasCarInformationModelNavigation?.CrMasSupCarModelEnName;
