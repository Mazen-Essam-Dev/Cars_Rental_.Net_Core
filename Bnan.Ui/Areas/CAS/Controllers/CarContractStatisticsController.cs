...
namespace Bnan.Ui.Areas.CAS.Controllers
{
    ...
    public class CarContractStatisticsController : BaseController
    {
        ...
        public async Task<IActionResult> GetAllByType(string Type ,string listDrop ,string singleNo ,string startDate,string endDate)
        {
            ...
            if (Type == "Model")
            {
                var AllCarContracts2_Model = _unitOfWork.CrCasRenterContractStatistic.FindAll(x => currentCar.CrMasUserInformationLessor == x.CrCasRenterContractStatisticsLessor, new[] { "CrCasRenterContractStatisticsModelNavigation", "CrCasRenterContractStatisticsBrandNavigation" }).Where(x => x.CrCasRenterContractStatisticsModelNavigation?.CrMasSupCarModelStatus != Status.Deleted).DistinctBy(x => x.CrCasRenterContractStatisticsModel).ToList();
                ...
                foreach (var single in AllCarContracts2_Model)
                {
                    ...
                    chartBranchDataVM.ArName_for_table_Model = single.CrCasRenterContractStatisticsBrandNavigation?.CrMasSupCarBrandArName + " - " + single.CrCasRenterContractStatisticsModelNavigation?.CrMasSupCarModelArName;
                    chartBranchDataVM.EnName_for_table_Model = single.CrCasRenterContractStatisticsBrandNavigation?.CrMasSupCarBrandEnName + " - " + single.CrCasRenterContractStatisticsModelNavigation?.CrMasSupCarModelEnName;
